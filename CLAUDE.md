...
A Rust engine for the board game **Hive (base game only)**. Eventual target is an AlphaZero-style self-play training pipeline, but the engine itself is the current deliverable.

The build proceeds in strict phases; each phase ends with a passing test suite before the next begins. Phases 1–4 (correctness, Zobrist hashing, make-unmake, incremental caches) are done; Phase 5 (profile-driven optimization) has shipped both the brief's candidates and a follow-up perf pass.
...
**Correct first, optimized second.** Each phase ends with the test suite green before the next begins. *Do not* build later-phase features early (no Zobrist in Phase 1, no incremental articulation maintenance in Phase 4 without a profile, etc.). When implementation reveals a conflict with a phase decision, **stop and raise it** rather than silently diverging.
